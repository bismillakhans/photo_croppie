...
def myprint(img, tot_noi, photo_size=[3.5, 4.5], paper_size=[6, 4], orienatation='L'):
    ...
    def inch2mm(inp):
        return np.round(np.multiply(inp, 25.4), 2)

    def inch2cm(inp):
        return np.round(np.multiply(inp, 2.54), 2)

    def inch2pxl(inp):
        return np.round(np.multiply(inp, 96))

    def pxl2inch(inp):
        return np.round(np.multiply(inp, 1 / 96), 2)

    def cm2inch(inp):
        return np.round(np.multiply(inp, 1 / 2.54), 2)

    def mm2inch(inp):
        return np.round(np.multiply(inp, 1 / 25.4), 2)

    def pxl2mm(inp):
        return np.round(np.multiply(inp, 25.4 / 96), 2)

    def mm2pxl(inp):
        return np.round(np.multiply(inp, 96 / 25.4))

    def pxl2cm(inp):
        return np.round(np.multiply(inp, 2.54 / 96), 2)
...
FACIAL_LANDMARKS_IDXS = {"mouth": (48, 68), "inner_mouth": (60, 68), "right_eyebrow": (17, 22),
                         "left_eyebrow": (22, 27), "right_eye": (36, 42), "left_eye": (42, 48), "nose": (27, 36),
                         "jaw": (0, 17)}
